...
// ⭐️ 발로란트 / 롤 티어 고유 색상 매핑
const VAL_COLORS = {
  "Iron": "#737373", "Bronze": "#A55D35", "Silver": "#A6B1B7", "Gold": "#D4AF37",
  "Platinum": "#37A1A4", "Diamond": "#B483D8", "Ascendant": "#2E8B57", 
  "Immortal": "#DC3545", "Radiant": "#FFF7B0", "Unranked": "#E0E0E0"
};

const LOL_COLORS = {
  "IRON": "#737373", "BRONZE": "#A55D35", "SILVER": "#A6B1B7", "GOLD": "#D4AF37",
  "PLATINUM": "#40E0D0", "EMERALD": "#28A745", "DIAMOND": "#4169E1", 
  "MASTER": "#8A2BE2", "GRANDMASTER": "#DC3545", "CHALLENGER": "#87CEFA", "Unranked": "#E0E0E0"
};
...
// ⭐️ 예전 스타일(네모 윤곽선)으로 돌아간 CustomDay
  ...
  return (
    <TouchableOpacity 
      style={[
        ...
        isToday && !isSelected && {borderWidth: 1.5, borderColor: theme.accent, borderRadius: 10}
      ]} 
...
export default function App() {
  ...
  const fetchBackendStats = async (gitId, rName, rTag, dateStr) => {
    try {
      ...
      setRawApiData(null);
      setApiStats([{ id: 'offline', title: '📴 오프라인 모드', desc: '인터넷 연결이 끊겼습니다.', sub: '다이어리는 로컬에 안전하게 저장됩니다.', bg: '#95A5A6' }]);
    }
  };
  ...
  const baseMarks = useMemo(() => {
    ...
    return marks;
  }, [displayedYear, getRecurringEventsForDate, records, THEME, systemMonthEvents]);
  ...
  const handleDayPress = useCallback((dateString) => {
    ...
    if (selectedDate === dateString) {
      if (records[dateString]) { 
        ...
        setCurrentDiary(''); setCurrentTodos([]); setCurrentPhotos([]);
      }
      setIsDiaryPreview(false);
  ...
  const combinedEvents = [...(systemMonthEvents[selectedDate] || []), ...todaySummaryEvents];
  ...
  // ⭐️ 스마트 필터링: Github (1 이상만), 발로란트/롤/롤체 (변화가 생긴 날만)
  const processGitData = useMemo(() => {
    ...
    return data.slice(-10); // 최근 10개만 보여줌
  }, [records]);
    ...
    <View style={[styles.container, {backgroundColor: THEME.bg}]}>
      ...
      <View style={[styles.calendarArea, {backgroundColor: THEME.bg}]}>
        <View style={styles.floatingHeader}>
          ...
          <TouchableOpacity style={[styles.headerButton]} onPress={() => { LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); setPickerYear(displayedYear); setMonthPickerVisible(true); }}>
            <CText fontScale={appFontScale} style={[styles.headerTitle, {color: THEME.text}]}>{displayedMonthName.split('-')[0]}년 {parseInt(displayedMonthName.split('-')[1])}월 ▼</CText>
          </TouchableOpacity>
          <View style={{flex: 1}} />
        </View>
        ...
        <Calendar
          ...
          hideExtraDays={true} // ⭐️ 모바일 공간 절약을 위해 지난달/다음달 회색 날짜 싹 지우기!
          onMonthChange={(m) => { 
            LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
      ...
      <View style={[styles.summaryArea, {backgroundColor: THEME.bgModal}]}>
        ...
        <ScrollView style={styles.summaryContent} showsVerticalScrollIndicator={false}>
          ...
          {combinedEvents.length === 0 && todaySummaryRecords.todos.length === 0 && !todaySummaryRecords.diary && todaySummaryRecords.photos?.length === 0 && (
            <View style={styles.emptyStateBox}>
              <CText fontScale={appFontScale} style={{color: THEME.textDim}}>일정이 없습니다. 두 번 눌러서 일기를 써보세요! ✨</CText>
            </View>
          )}
          ...
          {combinedEvents.map((ev, idx) => (
            <View key={idx} style={[styles.summaryItemBox, {backgroundColor: THEME.bg}]}>
              ...
              <CText fontScale={appFontScale} style={[styles.summaryItemText, {color: THEME.text}]}>
                {ev.isSystem ? '📱 ' : '✨ '}{ev.title}
              </CText>
            </View>
          ))}
              ...
              <ScrollView style={{maxHeight: height * 0.7}} showsVerticalScrollIndicator={false}>
                ...
                <View style={{flexDirection: 'row', marginBottom: 20}}>
                  {[{label: '작게', scale: 0.85}, {label: '보통', scale: 1.0}, {label: '크게', scale: 1.15}].map(opt => (
                    <TouchableOpacity key={opt.label} onPress={() => { LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); setAppFontScale(opt.scale); }} style={[styles.typeChip, {backgroundColor: THEME.bg, borderColor: '#eee'}, appFontScale === opt.scale && {backgroundColor: THEME.main, borderColor: THEME.main}]}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, backgroundColor: THEME.bgModal }}>
          <LinearGradient colors={[`${THEME.main}10`, THEME.bgModal]} style={styles.modalBgGradient}>
            ...
            <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
              ...
              <CText fontScale={appFontScale} style={[styles.sectionTitle, {color: THEME.text, marginBottom: 15}]}>📈 최근 기록 변화 타임라인</CText>
              ...
              <View style={{marginBottom: 20}}>
                ...
                {showGithub && processGitData.length > 0 && (
                  <View style={[styles.chartContainer, {width: width - 40}]}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timelineScroll}>
                      {processGitData.map((item, idx) => (
                        <View key={idx} style={styles.timelineItem}>
                          <CText fontScale={appFontScale} style={styles.timelineDate}>{item.date}</CText>
                          <View style={[styles.timelineSquare, { backgroundColor: '#28A745', opacity: Math.min(1, 0.4 + (item.val * 0.1)) }]} />
                          <CText fontScale={appFontScale} style={styles.timelineVal}>{item.val}개</CText>
                        </View>
                      ))}
                    </ScrollView>
                  </View>
                ...
                {/* ⭐️ 발로란트 (변화가 있는 날만 티어색상 네모로 렌더링) */}
                {showValorant && processValData.length > 0 && (
                  <View style={[styles.chartContainer, {width: width - 40}]}>
                    <CText fontScale={appFontScale} style={styles.chartTitle}>🔫 발로란트 티어 변화</CText>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timelineScroll}>
                      {processValData.map((item, idx) => (
                        <View key={idx} style={styles.timelineItem}>
                          <CText fontScale={appFontScale} style={styles.timelineDate}>{item.date}</CText>
                          <View style={[styles.timelineSquare, { backgroundColor: item.color }]} />
                          <CText fontScale={appFontScale} style={styles.timelineTier} numberOfLines={1}>{item.label.split(' ')[0]}</CText>
                          <CText fontScale={appFontScale} style={styles.timelineVal}>{item.val} RR</CText>
                        </View>
                      ))}
                    </ScrollView>
                  </View>
                ...
                {showLol && processLolData.length > 0 && (
                  <View style={[styles.chartContainer, {width: width - 40}]}>
                    <CText fontScale={appFontScale} style={styles.chartTitle}>⚔️ LoL 티어 변화</CText>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timelineScroll}>
                      {processLolData.map((item, idx) => (
                        <View key={idx} style={styles.timelineItem}>
                          <CText fontScale={appFontScale} style={styles.timelineDate}>{item.date}</CText>
                          <View style={[styles.timelineSquare, { backgroundColor: item.color }]} />
                          <CText fontScale={appFontScale} style={styles.timelineTier} numberOfLines={1}>{item.label.split(' ')[0]}</CText>
                          <CText fontScale={appFontScale} style={styles.timelineVal}>{item.val} LP</CText>
                        </View>
                      ))}
                    </ScrollView>
                  </View>
                ...
                {showTft && processTftData.length > 0 && (
                  <View style={[styles.chartContainer, {width: width - 40}]}>
                    <CText fontScale={appFontScale} style={styles.chartTitle}>♟️ TFT 티어 변화</CText>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timelineScroll}>
                      {processTftData.map((item, idx) => (
                        <View key={idx} style={styles.timelineItem}>
                          <CText fontScale={appFontScale} style={styles.timelineDate}>{item.date}</CText>
                          <View style={[styles.timelineSquare, { backgroundColor: item.color }]} />
                          <CText fontScale={appFontScale} style={styles.timelineTier} numberOfLines={1}>{item.label.split(' ')[0]}</CText>
                ...
                {(!showGithub && !showValorant && !showLol && !showTft) || 
                 (processGitData.length===0 && processValData.length===0 && processLolData.length===0 && processTftData.length===0) ? (
                   <View style={styles.emptyGraphBox}>
                     <CText fontScale={appFontScale} style={{color: THEME.textDim, textAlign: 'center', lineHeight: 22}}>
                       아직 데이터 변화 기록이 충분하지 않습니다.
                     </CText>
                   </View>
                ) : null}
              </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  calendarArea: { paddingTop: Platform.OS === 'ios' ? 40 : 15, zIndex: 2, paddingBottom: 0 },
  floatingHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 5, paddingHorizontal: 20 },
  headerButton: { backgroundColor: 'transparent', paddingVertical: 10, paddingHorizontal: 15 },
  headerTitle: { fontSize: 22, fontWeight: '900' },
  calendarStyle: { backgroundColor: 'transparent', paddingHorizontal: 5 },
  
  // ⭐️ 네모 형태의 날짜 셀 디자인 원복
  ...
  chartTitle: { fontSize: 16, fontWeight: 'bold', color: '#5B4D4D', marginBottom: 15 },
  
  // ⭐️ 타임라인(히트맵) 스타일
  timelineScroll: { flexDirection: 'row' },
  timelineItem: { alignItems: 'center', marginRight: 20 },
  timelineDate: { fontSize: 11, color: '#888', marginBottom: 8 },
  timelineSquare: { width: 34, height: 34, borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, elevation: 2 },
